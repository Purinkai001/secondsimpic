...
export default function ConfigPage() {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div>
                <h1 className="text-3xl font-bold bg-gradient-to-r from-foreground to-accent-blue bg-clip-text text-transparent">Game Configuration</h1>
                <p className="text-muted mt-1">Manage global rules and session behavior</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-surface-bg border border-surface-border p-8 rounded-3xl space-y-6">
                    ...
                    <form onSubmit={handleSave} className="space-y-4">
                        <div className="space-y-2">
                            <label className="text-sm text-muted">Default Question Timer (seconds)</label>
                            <input
                                type="number"
                                value={localTimer === "" ? config.questionTimer : localTimer}
                                onChange={(e) => setLocalTimer(e.target.value)}
                                className="w-full bg-surface-bg border border-surface-border rounded-xl px-4 py-3 focus:outline-none focus:border-accent-blue/50 transition-all font-medium text-foreground"
                            />
                        </div>

                        <button
                            disabled={saving}
                            className="w-full py-3 bg-accent-blue hover:bg-accent-blue/80 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                        >
                ...
                <div className="bg-surface-bg border border-surface-border p-8 rounded-3xl space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between p-4 bg-surface-bg/50 border border-surface-border rounded-2xl">
                            <div>
                                <p className="font-semibold">Trust-based Rejoining</p>
                                <p className="text-xs text-muted">Allows teams to reuse names if disconnected</p>
                            </div>
                            <div className="text-green-500 font-bold bg-green-500/10 px-3 py-1 rounded-lg text-xs">ENABLED</div>
                        </div>

                        <div className="flex items-center justify-between p-4 bg-surface-bg/50 border border-surface-border rounded-2xl">
                            <div>
                                <p className="font-semibold">Max Teams Per Group</p>
                                <p className="text-xs text-muted">Current limit: {config.maxTeamsPerGroup}</p>
                            </div>
                            <Users className="w-5 h-5 text-muted/40" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-red-500/5 border border-red-500/20 p-8 rounded-3xl space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                        <p className="text-sm text-muted">Reset all team scores and streaks to zero</p>
                        <button
                    ...
                    {/* Kick All */}
                    <div className="space-y-3">
                        <p className="text-sm text-muted">Remove all teams and their answer history</p>
                        <button
                ...
                <div className="space-y-3 pt-4 border-t border-red-500/10">
                    <p className="text-sm text-muted">Kick a specific team</p>
                    <div className="flex gap-3">
                        <select
                            value={selectedTeamId}
                            onChange={(e) => setSelectedTeamId(e.target.value)}
                            className="flex-1 bg-surface-bg border border-surface-border rounded-xl px-4 py-3 focus:outline-none focus:border-red-500/50 text-foreground appearance-none cursor-pointer overflow-x-auto whitespace-nowrap max-w-[200px] md:max-w-none"
                        >
