			...
			<AnimatePresence>
                {!isFullscreen && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-[999] bg-[#001439]/95 backdrop-blur-xl flex flex-col items-center justify-center p-8 text-center"
                    >
                        <Maximize className="w-24 h-24 text-[#b8a25f] mb-8 animate-pulse" />
                        <h2 className="text-4xl md:text-6xl font-atsanee font-bold text-white mb-6 uppercase tracking-wider">
                            Fullscreen Required
                        </h2>
                        <p className="text-xl md:text-2xl text-white/70 max-w-2xl mb-12">
                            To participate in SIMPIC 2026 Semifinals, please press <kbd className="px-3 py-1 bg-white/10 rounded-lg text-[#b8a25f] font-bold mx-2 border border-white/20 shadow-lg">F11</kbd> 
                            on your keyboard or click the button below to enter fullscreen mode.
                        </p>
                        <button
                            onClick={enterFullscreen}
                            className="px-10 py-5 bg-shiny text-[#001439] font-atsanee font-black text-3xl uppercase rounded-full hover:scale-105 active:scale-95 transition-transform shadow-[0_0_40px_rgba(184,162,95,0.4)]"
                        >
                            Enter Fullscreen
                        </button>
                    </motion.div>
			...
			<div className="relative h-[90vh] aspect-[16/9] bg-shiny p-[3px] rounded-[50px] shadow-2xl">
				<div className="relative w-full h-full bg-myBackground rounded-[50px] flex flex-col items-center justify-center p-8 overflow-hidden">
					...
					<div className="w-full max-w-5xl space-y-8 z-10">
						<form
							onSubmit={handleJoin}
							className="w-full max-w-5xl space-y-8 z-10"
						>
